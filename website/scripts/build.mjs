...
const deleteBody = `<section class="page-hero"><p class="eyebrow">Account &amp; data</p><h1>Delete your account</h1><p>Fire Feast includes a secure, authenticated deletion process inside the Game. There is no public account-deletion API on this website.</p></section>
<section class="content-card"><h2>Delete from the Game</h2><ol><li>Open Fire Feast on the device containing your guest account.</li><li>Open the <strong>Profile</strong> screen.</li><li>Under <strong>Account &amp; Data</strong>, choose the account-deletion control.</li><li>Review the warning and confirm the permanent deletion request.</li></ol><p>A successful request deletes the server-side guest account and associated data, then attempts to clear Fire Feast credentials and certain guest progression from that device. Deletion is permanent and the account or progress may not be recoverable.</p><h2>Can’t access the in-app control?</h2><p>Email <a href="mailto:${supportEmail}">${supportEmail}</a>. Include enough context to identify the issue, but do not send passwords, bearer tokens, guest authentication credentials, or recovery secrets.</p><p>For full details, read the <a href="/privacy">Privacy Policy</a>.</p></section>`;
