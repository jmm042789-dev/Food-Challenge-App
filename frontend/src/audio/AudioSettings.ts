...
export const DEFAULT_AUDIO_SETTINGS: AudioSettings = { master: 1, music: 0.28, soundEffects: 0.72, voice: 1, muted: false };
...
export async function loadAudioSettings(): Promise<AudioSettings> {
  ...
  try {
    ...
    return { master: clamp(stored.master, 1), music: clamp(stored.music, 0.28), soundEffects: clamp(stored.soundEffects, 0.72), voice: clamp(stored.voice, 1), muted: stored.muted === true };
...
export async function saveAudioSettings(settings: Partial<AudioSettings>): Promise<AudioSettings> {
  ...
  const next = { master: clamp(settings.master, current.master), music: clamp(settings.music, current.music), soundEffects: clamp(settings.soundEffects, current.soundEffects), voice: clamp(settings.voice, current.voice), muted: settings.muted ?? current.muted };
